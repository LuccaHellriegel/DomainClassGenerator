...
const commentToStr = (comment?: string) => (comment ? "//" + comment + "\n" : "");

export const annotationToStr = (annotation: string) => "@" + annotation;
const annotationsToStr = (annotations: string[]) => annotations.map((a) => annotationToStr(a) + "\n").join("");

const paramToStr = (param: JavaParam) => param.type + " " + param.name;

const modifiersToStr = (modifiers: Modifier[]) => modifiers.join(" ");

export const classFieldToStr = (field: ClassField) =>
...
export const methodSignatureToStr = (sig: MethodSignature) =>
...
export const classModelToStr = (model: ClassModel): string => {
	const head =
		commentToStr(model.comment) + annotationsToStr(model.annotations) + "public class " + model.name + " {\n";
